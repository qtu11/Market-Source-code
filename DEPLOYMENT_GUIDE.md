...
## ☁️ Cloudflare Workers (OpenNext)

1. Cài OpenNext và build worker:
   ```bash
   npm i -D opennext
...
> Các route cần Node APIs (pg, firebase-admin…) sẽ hoạt động bình thường trên Workers nhờ `nodejs_compat`, không cần ép về Edge runtime.
