    ...
    if (isAdmin) {
      ...
      const adminResult = await pool.query(
        `SELECT id as user_id FROM users WHERE role = 'admin' LIMIT 1
         UNION
         SELECT user_id as user_id FROM admin LIMIT 1
         LIMIT 1`
      );
      ...
      if (adminResult.rows.length > 0) {
        ...
        // ✅ FIX: Nếu không tìm thấy admin, trả về lỗi rõ ràng
        return NextResponse.json({ 
          success: false, 
          error: 'Admin not found. Vui lòng liên hệ quản trị viên để tạo tài khoản admin.' 
        }, { status: 404 });
      }
    }
