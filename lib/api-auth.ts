    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      if (canUseEmailAuth && userEmail) {
        ...
        try {
          ...
          const rateLimit = await checkRateLimit(rateLimitKey, 5, 60); // 5 requests/minute
...
export async function requireAdmin(request: NextRequest) {
  ...
  // Check admin token từ cookie hoặc header
  const adminToken = request.cookies.get('admin-token')?.value 
    || request.headers.get('X-Admin-Token');

  if (!adminToken) {
    // Fallback: check X-Admin-Auth header (legacy)
    const legacyAdminAuth = request.headers.get('X-Admin-Auth');
    if (legacyAdminAuth === 'true') {
      // Verify user is actually admin by checking database
      const { getUserIdByEmail } = await import('@/lib/database');
      const userId = await getUserIdByEmail(user.email || '');
      
      if (userId) {
        const { pool } = await import('@/lib/database');
        // ✅ FIX: Check cả admin table và users.role = 'admin'
  ...
  // Verify admin token (JWT)
  try {
    const { verifyAdminToken } = await import('@/lib/jwt');
    const decoded = await verifyAdminToken(adminToken);
    ...
    // Fallback: check X-Admin-Auth header
    const legacyAdminAuth = request.headers.get('X-Admin-Auth');
    if (legacyAdminAuth === 'true') {
      const { getUserIdByEmail } = await import('@/lib/database');
      const userId = await getUserIdByEmail(user.email || '');
