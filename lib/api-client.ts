    ...
    if (!user) {
      ...
      try {
        ...
        if (savedUser) {
          ...
          console.warn('User in localStorage but not in Firebase Auth');
          return null;
        }
      } catch (storageError) {
    ...
    if (isLoggedIn) {
      ...
      if (!savedUser && typeof window !== 'undefined') {
        try {
          ...
          console.warn('⚠️ Error looping localStorage:', loopError);
        }
      }
      ...
      if (savedUser) {
        try {
          ...
          if (user.email && typeof user.email === 'string' && user.email.includes('@')) {
            ...
            if (!token) {
              console.log('📧 Using email-based auth (no token):', user.email);
            }
          } else {
            console.warn('⚠️ User email not found or invalid in localStorage:', user);
          }
        } catch (parseError) {
        ...
        if (!token) {
          console.warn('⚠️ No user data found in localStorage for email-based auth');
        }
      }
    }
  } catch (storageError) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        if (isLoggedIn) {
          ...
          console.warn('401 Unauthorized - User may need to re-login', {
            endpoint,
            hasUser: !!savedUser,
            userEmail
          });
